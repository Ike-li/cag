...
## 实战发现 #1：provider 副产物（__pycache__）
- 发现来源：Rung 1, 2
- 级别：低
- 状态：已明确
- 说明：codex/agy 自我验证留 __pycache__；项目 .gitignore 会挡，无则 git add -A 全收；真仓库有 .gitignore，实害有限。
...
- 级别：低
- 状态：未触发
- 说明：哨兵比对 git status --porcelain；若 provider 越狱后在主仓库 git add && commit，工作树 clean 前后快照相等会漏报；理论存在，本次未触发。
