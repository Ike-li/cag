...
- 修改 `bin/cag-exec` 是咽喉脚本，需谨慎：它集中了 worktree 校验、env strip、provider 执行、越狱哨兵（前后快照对比抓 provider 改主仓库）和 git commit。
- 软链接是部署方式：`~/.claude/skills/cag`、`~/.claude/agents/*.md`、`~/.local/bin/cag-exec` 指向本仓库。

## Three Invariants

- **执行权在 provider**（codex/agy 真改代码）
- **隔离在 worktree + cag-exec 多层防御**：路径校验 + agy `--add-dir`/`--sandbox` + 越狱哨兵（provider 逃出 worktree 改主仓库 → 拒绝 + exit 3）
